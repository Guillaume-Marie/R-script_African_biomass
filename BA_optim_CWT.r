...
library("mixdist")  
# Create the function.
conn <- dbConnect(RSQLite::SQLite(), "temp_bm_02.gpkg")
dd2=dbGetQuery(conn,"SELECT LC_majority, MAP_mean, BM_majority FROM temp_bm_02 WHERE LC_variety==1 AND LC_majority IN (10,11,30,40,50,60,61,62,100,110,120,130,150,153,160)", params = 0)
ytot=na.omit(dd2)
ybio=subset(ytot, BM_majority>0)  
...
prior <- read.table("prior_Fraction.csv",dec=",",sep=" ",header=TRUE)
...
for(i in 1:15) {
    ...
    assign(paste0("out.CWT",i),bugs(data,inits=NULL,params,model.file,codaPkg=TRUE, 
